...
(function () {
  ...
  function doSignIn(rawUser, pass) {
    ...
    if (VALID_MEMBERS[u] && VALID_MEMBERS[u] === pass) { setUser(u); return true; }
    try {
      var m = JSON.parse(localStorage.getItem('mx_joined') || '{}');
      if (m[u] && m[u] === pass) { setUser(u); return true; }
    } catch (e) {}
    return false;
  }
  ...
  function doJoin(rawUser, pass) {
    ...
    var m = {};
    try { m = JSON.parse(localStorage.getItem('mx_joined') || '{}'); } catch (e) {}
    if (m[u]) return 'taken';
    m[u] = pass;
    ...
    fetch('https://matix-1d538-default-rtdb.firebaseio.com/profiles/' + encodeURIComponent(u) + '.json', {
      ...
      body: JSON.stringify({ displayName: u, bio: '', ideasText: '', memesText: '', updatedAt: Date.now() })
    }).catch(function () {});
    return 'ok';
  }
  ...
  function vd() { return mk('div', 'mx-vd'); }
  ...
  var CSS = '\n' + [
    ...
    '#mx-menu-toggle:hover{filter:brightness(1.06)}',
    ...
    '.mx-nb:hover,.mx-nb.mx-open{color:#fff;background:rgba(255,255,255,0.06);border-bottom-color:#f59e0b;text-decoration:none}',
    ...
    '.mx-join-btn:hover,.mx-join-btn.mx-open{background:rgba(16,185,129,.22)}',
    ...
    '.mx-si-btn:hover,.mx-si-btn.mx-open{background:rgba(245,158,11,.20)}',
    '.mx-si-btn.mx-signed{background:rgba(59,130,246,.08);border-color:#3b82f6;color:#93c5fd}',
    '.mx-si-btn.mx-signed:hover{background:rgba(59,130,246,.20)}',

    /* panel base */
    '.mx-panel{position:fixed;top:66px;background:rgba(2,7,22,0.98);border:1px solid rgba(59,130,246,.18);border-top:3px solid #f59e0b;border-radius:0 0 14px 14px;box-shadow:0 32px 80px rgba(0,0,0,.90);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);z-index:9999;opacity:0;pointer-events:none;transform:translateY(-8px);transition:opacity .16s ease,transform .16s ease;overflow:hidden}',
    '.mx-panel.mx-open{opacity:1;pointer-events:auto;transform:translateY(0)}',

    /* iframe preview panel */
    '.mx-p-iframe{width:660px;height:430px;position:relative;overflow:hidden}',
    '.mx-p-iframe iframe{width:1320px;height:860px;transform:scale(.5);transform-origin:0 0;border:none;pointer-events:none;display:block;background:#030d1e}',
    '.mx-ifr-foot{position:absolute;bottom:0;left:0;right:0;padding:9px 14px;background:linear-gradient(to top,rgba(2,7,22,.98) 55%,transparent);display:flex;align-items:flex-end;justify-content:space-between}',
    ...
    '.mx-ifr-open:hover{background:rgba(245,158,11,.18);color:#fbbf24;text-decoration:none}',
    ...
    '.mx-p-sub a:hover{background:rgba(59,130,246,.14);color:#fff;border-left-color:#f59e0b;text-decoration:none}',
    ...
    '.mx-switch-hint a:hover{text-decoration:underline}',
    ...
    '.mx-profile-link:hover{background:rgba(59,130,246,.14);text-decoration:none}',
    ...
    '.mx-lesson-row:hover{background:rgba(59,130,246,.08)}',
    ...
    '.mx-lesson-by:hover{text-decoration:underline}',
    ...
    '.mx-lbtn.v:hover{background:rgba(59,130,246,.18)}',
    '.mx-lbtn.e{border-color:rgba(245,158,11,.30);color:#fbbf24}',
    '.mx-lbtn.e:hover{background:rgba(245,158,11,.18)}',
    ...
    '.mx-pl-foot .va:hover{background:rgba(59,130,246,.18);text-decoration:none}',
    '.mx-pl-foot .ex{border-color:rgba(96,165,250,.20);color:#7ea6d4}',
    '.mx-pl-foot .ex:hover{background:rgba(59,130,246,.12);text-decoration:none}',
    '.mx-pl-foot .cr{border-color:#10b981;color:#34d399}',
    '.mx-pl-foot .cr:hover{background:rgba(16,185,129,.18)}',
    ...
    '.mx-pf-back:hover{color:#93c5fd}',

    /* media */
    '@media(max-width:768px){#mx-menu-toggle{top:8px;left:8px;width:48px;height:48px}#mx-nav-bar{top:8px;left:62px;right:8px;height:48px;padding:0 10px}body{padding-top:66px!important}.mx-nb{height:48px;padding:0 9px;font-size:.8rem}.mx-panel{display:none!important}}'
  ].join('\n');
  ...
  function hidePanels() {
    allPanels.forEach(function (p) { p.classList.remove('mx-open'); });
    document.querySelectorAll('#mx-nav-bar .mx-open').forEach(function (b) { b.classList.remove('mx-open'); });
    activePanel = null;
  }
  ...
  function positionPanel(panel, trigger, width) {
    ...
    var w = width || 220;
    var left = r.left;
    left = Math.max(0, Math.min(left, window.innerWidth - w - 8));
  ...
  function wireHover(id, panel, trigger, width, onBefore) {
    var t = null;
    ...
    trigger.addEventListener('mouseleave', function () {
      t = setTimeout(function () { if (activePanel === id) hidePanels(); }, 160);
    });
  ...
  logoImg.onerror = function () {
    ...
    var wm = mk('span', 'mx-logo-wordmark');
    wm.innerHTML = 'MA<em>T</em>IX';
    logo.appendChild(wm);
  };
  logo.appendChild(logoImg);
  var logoWm = mk('span', 'mx-logo-wordmark');
  logoWm.innerHTML = 'MA<em>T</em>IX';
  logo.appendChild(logoWm);
  ...
  LINKS.forEach(function (link, i) {
    ...
    if (link.iframe) {
      ...
      panel._ifr = ifr; panel._ifrSrc = link.href; panel._ifrLoaded = false;
      wireHover(pid, panel, btn, 660, function () {
        if (!panel._ifrLoaded) { panel._ifr.src = panel._ifrSrc; panel._ifrLoaded = true; }
      });
      ...
      link.subs.forEach(function (sub, si) {
        ...
        var a = mk('a'); a.href = sub.href; a.textContent = sub.label;
        a.addEventListener('click', hidePanels);
        panel.appendChild(a);
      });
      wireHover(pid, panel, btn, 212);

    } else {
      panel.classList.add('mx-p-sub');
      var a2 = mk('a'); a2.href = link.href; a2.textContent = '\u2192  ' + link.label;
      a2.addEventListener('click', hidePanels);
      panel.appendChild(a2);
      wireHover(pid, panel, btn, 192);
    }
  ...
  function renderLessons() {
    ...
    var ht = mk('h3'); ht.textContent = 'Lessons'; hd.appendChild(ht);
    ...
    if (!lsLoaded) {
      ...
      fetch('https://matix-1d538-default-rtdb.firebaseio.com/lessons.json')
        ...
        .then(function (d) { lsLoaded = true; lsData = d || {}; fillLessons(bd, user); })
        .catch(function () { bd.innerHTML = '<div class="mx-pl-empty">Could not load lessons.</div>'; });
    } else {
      fillLessons(bd, user);
    }

    var ft = mk('div', 'mx-pl-foot');
    var va = mk('a', 'va'); va.href = '/lessons'; va.textContent = 'All Lessons'; va.addEventListener('click', hidePanels); ft.appendChild(va);
    var ex = mk('a', 'ex'); ex.href = '/lessons/exercises'; ex.textContent = '\ud83d\udcdd Exercises'; ex.addEventListener('click', hidePanels); ft.appendChild(ex);
    if (user) {
      var cr = mk('button', 'cr'); cr.textContent = '+ Create';
  ...
  function fillLessons(bd, user) {
    ...
    if (!keys.length) { bd.innerHTML = '<div class="mx-pl-empty">No lessons yet \u2014 be the first!</div>'; return; }
    keys.forEach(function (id) {
      ...
      byEl.addEventListener('click', function () { hidePanels(); window.location.href = '/users/' + lessonOwner; });
      var nm = mk('div', 'mx-lesson-name'); nm.textContent = d.title || 'Untitled';
      meta.appendChild(byEl); meta.appendChild(nm); row.appendChild(meta);

      var btns = mk('div', 'mx-lesson-btns');
      var vb = mk('button', 'mx-lbtn v'); vb.textContent = 'View';
      vb.addEventListener('click', function () { hidePanels(); window.location.href = '/lessons'; });
      btns.appendChild(vb);

      if (user && (d.createdBy === user || user === 'ghadi')) {
        var eb = mk('button', 'mx-lbtn e'); eb.textContent = '+ Ex';
        eb.addEventListener('click', (function (lid, lt) {
          return function () { showAddExercise(lid, lt); };
        }(id, d.title || 'Untitled')));
        btns.appendChild(eb);
      }
  ...
  function showCreateLesson() {
    lessonsPanel.innerHTML = '';
    var f = mk('div', 'mx-pf');
    var bk = mk('button', 'mx-pf-back'); bk.textContent = '\u2190 Back';
    bk.addEventListener('click', function () { lsLoaded = false; renderLessons(); });
    f.appendChild(bk);
    var h4 = mk('h4'); h4.textContent = 'Create Lesson'; f.appendChild(h4);
    var ti = mk('input', 'mx-fi'); ti.type = 'text'; ti.placeholder = 'Title'; ti.maxLength = 80; f.appendChild(ti);
    var de = mk('textarea', 'mx-fi ta'); de.placeholder = 'Description (optional)'; f.appendChild(de);
    var err = mk('div', 'mx-ferr'); f.appendChild(err);
    var sb = mk('button', 'mx-pb g'); sb.textContent = 'Publish';
    sb.addEventListener('click', function () {
      var ttl = ti.value.trim();
      if (!ttl) { err.textContent = 'Enter a title.'; return; }
      sb.textContent = 'Publishing\u2026'; sb.disabled = true;
      fetch('https://matix-1d538-default-rtdb.firebaseio.com/lessons.json', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: ttl, description: de.value.trim(), createdBy: getUser(), createdAt: Date.now() })
      }).then(function () { lsLoaded = false; renderLessons(); })
        .catch(function () { err.textContent = 'Failed. Try again.'; sb.textContent = 'Publish'; sb.disabled = false; });
    });
    f.appendChild(sb);
  ...
  function showAddExercise(lid, ltitle) {
    ...
    var f = mk('div', 'mx-pf');
    var bk = mk('button', 'mx-pf-back'); bk.textContent = '\u2190 Back';
    bk.addEventListener('click', renderLessons); f.appendChild(bk);
    var h4 = mk('h4'); h4.textContent = 'Add Exercise'; f.appendChild(h4);
    var sub = mk('p', 'mx-pf-sub'); sub.textContent = ltitle; f.appendChild(sub);
    var qi = mk('input', 'mx-fi'); qi.type = 'text'; qi.placeholder = 'Question'; qi.maxLength = 300; f.appendChild(qi);
    var ai = mk('input', 'mx-fi'); ai.type = 'text'; ai.placeholder = 'Answer'; ai.maxLength = 300; f.appendChild(ai);
    var msg = mk('div', 'mx-ferr'); f.appendChild(msg);
    var sb = mk('button', 'mx-pb y'); sb.textContent = 'Add Exercise';
    sb.addEventListener('click', function () {
      var q = qi.value.trim(), a = ai.value.trim();
      if (!q || !a) { msg.textContent = 'Fill both fields.'; return; }
      sb.textContent = 'Saving\u2026'; sb.disabled = true;
      fetch('https://matix-1d538-default-rtdb.firebaseio.com/exercises/' + lid + '.json', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ question: q, answer: a, createdBy: getUser(), createdAt: Date.now() })
      }).then(function () {
        msg.className = 'mx-fok'; msg.textContent = 'Added!';
        qi.value = ''; ai.value = ''; sb.textContent = 'Add Exercise'; sb.disabled = false;
      }).catch(function () { msg.textContent = 'Failed.'; sb.textContent = 'Add Exercise'; sb.disabled = false; });
    });
    f.appendChild(sb);
    lessonsPanel.appendChild(f);
  }
  ...
  function buildJoinPanel() {
    ...
    if (user) {
      ...
      sb.addEventListener('click', function () {
        ...
        if (res === 'short')    { er.textContent = 'Username must be at least 2 characters.'; return; }
        if (res === 'reserved') { er.textContent = 'That username is reserved \u2014 use Sign In.'; return; }
        if (res === 'taken')    { er.textContent = 'Username taken. Choose another.'; return; }
        refreshAuth(); buildJoinPanel(); buildSiPanel();
      });
      [ui, pi, p2].forEach(function (inp) { inp.addEventListener('keydown', function (e) { if (e.key === 'Enter') sb.click(); }); });
      var hint = mk('p', 'mx-switch-hint');
      hint.innerHTML = 'Already have an account? <a id="mx-gsi">Sign In \u2192</a>';
      w.appendChild(ui); w.appendChild(pi); w.appendChild(p2); w.appendChild(er); w.appendChild(sb); w.appendChild(hint);
    }
    joinPanel.appendChild(w);
    var gsi = joinPanel.querySelector('#mx-gsi');
    if (gsi) gsi.addEventListener('click', function () { hidePanels(); buildSiPanel(); openPanel('si', siPanel, siBtn, 290); });
  }

  wireHover('join', joinPanel, joinBtn, 290, buildJoinPanel);
  ...
  siPanel.id = 'mxp-si';
  document.body.appendChild(siPanel);
  ...
  function buildSiPanel() {
    ...
    var w = mk('div', 'mx-p-auth');
    if (user) {
      renderUserCard(w, user, function () { doSignOut(); refreshAuth(); buildSiPanel(); buildJoinPanel(); });
    } else {
      var h3 = mk('h3'); h3.textContent = 'Sign In'; w.appendChild(h3);
      var ui = mk('input', 'mx-fi'); ui.type = 'text'; ui.placeholder = 'Username'; ui.autocomplete = 'username';
      var pi = mk('input', 'mx-fi'); pi.type = 'password'; pi.placeholder = 'Password'; pi.autocomplete = 'current-password';
      var er = mk('div', 'mx-ferr');
      var sb = mk('button', 'mx-pb y'); sb.textContent = 'Sign In';
      sb.addEventListener('click', function () {
        if (doSignIn(ui.value, pi.value)) { refreshAuth(); buildSiPanel(); buildJoinPanel(); }
        else { er.textContent = 'Invalid username or password.'; }
      });
    ...
    if (gjo) gjo.addEventListener('click', function () { hidePanels(); buildJoinPanel(); openPanel('join', joinPanel, joinBtn, 290); });
  }

  wireHover('si', siPanel, siBtn, 290, buildSiPanel);

  function renderUserCard(container, user, onSignOut) {
    var uc = mk('div', 'mx-user-wrap');
    var un = mk('div', 'mx-uname-big'); un.textContent = '\ud83d\udc64 ' + user;
    ...
    var so = mk('button', 'mx-pb r'); so.textContent = 'Sign Out'; so.style.marginTop = '6px';
    so.addEventListener('click', onSignOut);
    uc.appendChild(un); uc.appendChild(ul); uc.appendChild(profLink); uc.appendChild(so);
  ...
  function syncPanelTop() {
    var top = menuToggle.getBoundingClientRect().bottom + 4;
  ...
  function inject() {
    ...
    document.addEventListener('click', function (e) {
      ...
      if (allPanels.some(function (p) { return p.contains(e.target); })) return;
      closeMenu();
    });
